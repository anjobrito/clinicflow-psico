...
type Note = { appointmentId: string; content: string; finalized: boolean; };
...
function addDays(base: Date, days: number) { const date = new Date(base); date.setDate(date.getDate() + days); return date; }
function toIso(date: Date) { return date.toISOString().slice(0, 10); }
function monthDays(base: Date) { const first = new Date(base.getFullYear(), base.getMonth(), 1); return Array.from({ length: 35 }, (_, index) => addDays(first, index)); }
...
export default function Home() {
  ...
  const filteredDayAppointments = useMemo(() => dayAppointments.filter((item) => (statusFilter === "Todos" || item.status === statusFilter) && patientName(item.patientId).toLowerCase().includes(search.toLowerCase())), [dayAppointments, search, statusFilter]);
  ...
  function patientName(id: string) { return patients.find((patient) => patient.id === id)?.name || "Paciente não encontrado"; }
  function statusClass(status: Status) { if (status === "Atendida") return "badge badge-green"; if (status === "Faltou" || status === "Cancelada") return "badge badge-red"; if (status === "Remarcada" || status === "Em atendimento") return "badge badge-yellow"; return "badge badge-blue"; }
  ...
  function createAppointment(event: FormEvent) {
    event.preventDefault();
    const item: Appointment = { id: `a${Date.now()}`, patientId: appointmentForm.patientId, date: appointmentForm.date, time: appointmentForm.time, duration: Number(appointmentForm.duration), professional: appointmentForm.professional, room: appointmentForm.room, mode: appointmentForm.mode, status: appointmentForm.status, value: Number(appointmentForm.value || 0) };
  ...
  function setAppointmentStatus(item: Appointment, status: Status) { setAppointments((current) => current.map((row) => row.id === item.id ? { ...row, status } : row)); }
  function saveNote(finalized: boolean) { if (!selectedAppointment) return; setNotes((current) => [{ appointmentId: selectedAppointment.id, content: editor, finalized }, ...current.filter((note) => note.appointmentId !== selectedAppointment.id)]); if (finalized) setAppointmentStatus(selectedAppointment, "Atendida"); }
